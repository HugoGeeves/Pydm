...
from execution.parampop import generator_form, car_form, truck_form, simulation_form, inhom_form, specific_inhom_form, \
    point_detector_form, specific_point_detector_form, space_detector_form, specific_space_detector_form, platoon_form, \
    bridge_detector_form
...
class MainWindow(object):
    def __init__(self):
        ...
        self.fileMenu.add_command(label="Platoon", command=self.platoon_parameters)
        ...
        self.spaceMenu = Menu(menubar)
        self.spaceMenu.add_command(label="Bridge Detector", command=self.bridge_detector_parameters)
        menubar.add_cascade(label="Bridge Detector", menu=self.spaceMenu)
        ...
        if self.animation_state == "PLAY" and self.graphical.get():
            ...
            for vehicle in vehicles:
                if vehicle.__class__.__name__ is not "Platoon":
                    ...
                    for i in range(0,int(vehicle.platoon_size)):
                        self.canvas.create_rectangle(
    ...
    def platoon_parameters(self):

        win = Toplevel()
        platoon_form(self.sim, win)
    ...
    def bridge_detector_parameters(self):

        win = Toplevel()
        bridge_detector_form(self.sim, win)

    def sim_end(self):
        current_datetime = time.strftime("%Y%m%d-%H%M")
        filename = "Results" + current_datetime + ".xlsx"
        ...
        worksheet = workbook.add_worksheet("Bridge Detector")
        row = 0
        col = 0

        agg_data = self.sim.bridge_detector[0].recorded_loads
        for record in agg_data:
